...
class FormatterCommand: NSObject, XCSourceEditorCommand {
    ...
    private func createTemporayFolder() throws -> URL? {
        ...
        let temporaryFolderURL = rootURL.appendingPathComponent(
            UUID().uuidString,
            isDirectory: true
        )
    ...
    private func taskDidTerminated(
        _ terminationStatus: Int32,
        ...
        do {
            if terminationStatus == 0 {
                ...
                if selectedLineRange.location != NSNotFound {
                    ...
                    if outputLines.count > 0 {
                        invocation.buffer.lines.replaceObjects(
                            in: selectedLineRange,
                            withObjectsFrom: outputLines
                        )
                ...
                if let errorData = task?.standardErrorHandle()?.readDataToEndOfFile(),
                   let print = String(
                       data: errorData,
                       encoding: .utf8
                   )
                {
                    ...
                    throw FormatterError
                        .failure(reason: "Uncrustify error — exit code \(terminationStatus)")
                }
            }
        } catch {
            didFailedPerformCommand(error)
    ...
    private func makePathExtension(uti: String) -> String {
        if let ext = UTTypeCopyPreferredTagWithClass(
            uti as CFString,
    ...
    private func makeTask(
        with execType: Executable.Type,
        invocation: XCSourceEditorCommandInvocation
    ) throws -> Process {
        let uti = invocation.buffer.contentUTI
        let isFragmented = invocation.commandIdentifier
            .hasSuffix(FormatAction.formatSelctedLines.rawValue)
        ...
        let sourceFileURL = temporaryFolderURL!.appendingPathComponent(
            sourceFileName,
            isDirectory: false
        )
        ...
        if isFragmented {
            ...
            selectedLineRange = NSMakeRange(
                selectedTextRange.start.line,
                selectedTextRange.end.line - selectedTextRange.start.line + 1
            )
            ...
            try invocation.buffer.completeBuffer.write(
                to: sourceFileURL,
                atomically: true,
                encoding: .utf8
            )
        ...
        let args = try execType.makeTaskArgs(
            uti: uti,
            isFragmented: isFragmented,
            sourceFile: sourceFileURL.path
        )
